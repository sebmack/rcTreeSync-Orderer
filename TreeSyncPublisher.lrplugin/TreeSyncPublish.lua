...
function TreeSyncPublish:startDialogMethod( props )
	...
	view:setObserver( props, 'treeData', TreeSyncPublish, FtpPublish.propertyChangeHandler )
...
function TreeSyncPublish:computeSourcePathOptions( props )
    ...
    app:call( Call:new { name="computeSourcePathOptions", async=true, guard=App.guardSilent, main=function( context )
        ...
        LrFunctionContext.callWithContext( 'Source Root Selection', function( context )
            ...
            --     local rootCollectionSets = catalog:getChildCollectionSets()
            --     pt.collectionTree = {}

            --     local function checkStatus(treeNodes, currentPath)
            --         for _, node in ipairs(treeNodes) do
            --             local nextPath = currentPath .. node.label
            --             pt[nextPath] = node.checked
            --             pt[nextPath .. "collapsed"] = node.collapsed

            --             -- LrDialogs.message("checkStatus full path: ", nextPath .. tostring(node.checked))
            --             if #node.children then
            --                 checkStatus(node.children, nextPath)
            --             end
            --         end
            --     end

            --     for _, rootSet in ipairs(rootCollectionSets) do
            --         local rootNode = {
            --             label = rootSet:getName(),
            --             checked = false,
            --             collapsed = true,  -- Start with all items collapsed
            --             children = {},
            --         }
            --         props.sourcePath = rootNode.label
            --         pt[rootNode.label] = false
            --         pt[rootNode.label .. "collapsed"] = false
            --         buildCollectionTree(rootSet, rootNode, rootNode.label)
            --         table.insert(pt.collectionTree, rootNode)
            --     end

            --     local isSetCollSetId = true
            --     local collSetInfos = {}
            --     tab:appendArray( collSetInfos, cat:getCollectionSetPopupItems( props.isPubColl ) )
            --     for _, dataItem in ipairs(collSetInfos) do
            --         for _key, _val in pairs(dataItem) do
            --             -- LrDialogs.message("props.collSetId: " .. tostring(_key),  tostring(_val))
            --             if isSetCollSetId and _key == "value" then
            --                 props.collSetId = _val
            --                 isSetCollSetId = false
            --             end
            --         end
            --     end


            --     if props.treeData and #props.treeData then
            --         pt.collectionTree = props.treeData
            --         checkStatus(pt.collectionTree, "")
            --     end
            ...
            --                 local nextPath = currentPath .. childNode.label
            ...
            --                     },
            --                     vf:push_button {
            --                         title = node.collapsed and "▼" or "▶",
            --                         width = 20,
            --                         action = function()
            --                             node.collapsed = not node.collapsed
            --                             pt[nextPath .. "collapsed"] = node.collapsed
            --                             -- LrDialogs.message("props treeData: ", node.label)
            --                         end,
            --                     },
            --                     vf:checkbox {
            --                         title = "",
            ...
            --                             checkAllSubCollections(node, node.checked, nextPath)
            --                         end,
            --                     },
            --                     vf:static_text {
            --                         title = node.label,
            ...
            --             if node.collapsed and node.children then
            ...
            --         return copy
            --     end
            --     pt.originalCollectionTree = deepCopy(pt.collectionTree)
            --     pt.treeData = renderTree(pt.collectionTree, 0, "")
            --     local contents = vf:column {
            --         bind_to_object = pt,
            --         vf:scrolled_view {
            --             width = 400,
            --             height = 300,
            --             vf:column(pt.treeData),
            --         },
            --     }
            ...
            --         -- LrDialogs.message("source path: ", props.sourcePath)
            --         if #pt.collectionTree then
            --             props.treeData = pt.collectionTree
            --         end
            --     elseif button == 'cancel' then
            --         props.treeData = pt.originalCollectionTree
            --     end
...
function TreeSyncPublish:markAsPublishedForMissingOriginals( call, photos, collection, service )
    local markAsPublished = true
    local serviceName = "Mark as Published"
    local pubSrvs = PublishServices:new()
    --pubSrvs:init( call ) -- I'm not sure if other plugin-info is needed, but better safe than sorry.. ###4 'til 15/Sep/2014 16:21 - all pub srvs. remove this line/reminder if no problems by 2016.
    pubSrvs:init( call, _PLUGIN.id ) -- theoretically, this feature should not care a hoot what info there is or is not for other publish services, I mean it could be that
        -- it would make a differences as to logged info (e.g. "not published at all" vs. "not published on this service"), so check it, but I want it to be faster, and therefore limited to TSP services.
    local pubSrv = service

    local pubCollSet = {} -- selected publish collections
    local pubColls = {} -- ditto, as array
    
    local vType = cat:getSourceType( collection )
    if vType == 'LrPublishedCollection' then
        pubCollSet[collection] = true
        pubColls[#pubColls + 1] = collection
    end
    if tab:isEmpty( pubCollSet ) then
        app:show{ warning="Select one or more publish collections, then try again." }
        return true
    ...
    if getRemoteUrl == nil then
        getRemoteUrl = function()
            return true, "No function for getting remote URL in advanced settings."
        end
        app:logV( "Using default getRemoteUrl function - consider configuring using advanced settings - preset manager section of plugin manager." )
    end

    -- LrDialogs.message(service:getName(), collection:getName().."\n"..tostring(collection).."\n"..tostring(#photos))
    ...
    for i, photo in ipairs( photos ) do
        repeat
            ...
            --     local pubPhotosToDo = {}
            --     local pubSrvSettings = {}
            --     -- LrDialogs.message(collection, photoName )
            --     for pubColl, v in pairs( photoInfo.pubCollSet ) do
            --         LrDialogs.message(photoName, pubColl:getName().."\n"..tostring(pubCollSet[pubColl]) )
            --         if pubCollSet[pubColl] then -- this published collection that photo is in, is one that's selected.
            --             pubCollsToDo[#pubCollsToDo + 1] = pubColl
            --             if not markAsPublished then -- we need eligible published photos only.
            --                 for i, pubPhoto in ipairs( photoInfo.pubPhotos ) do
            ...
            --             pubCollsToDo[#pubCollsToDo + 1] = collection
            --         else
            --             app:logV( "*** mark as publish with foreign publish collection selected?" ) 
            --         end
            --     else
    ...
    if #todo > 0 then
        call:setCaption( "Updating catalog..." )
        local s, m = cat:update( 50, serviceName, function( context, phase )
            local i1 = ( phase - 1 ) * 1000 + 1
            local i2 = math.min( phase * 1000, #todo )
            app:logV( "Updating photos from ^1 to ^2", i1, i2 )
            for i = i1, i2 do
                ...
                if call:isQuit() then
                    ...
                    call:setPortionComplete( i, #todo )
                    if i == #todo then
                        call:setCaption( "Finalizing..." )
                    end
                end
            end
            if i2 < #todo then
...
function TreeSyncPublish:checkBeforeRendering()
    -- LrDialogs.message("checkBeforeRendering")
...
exportParams[#exportParams + 1] = { key = 'treeData', default = nil }
