...
-- Check we have an exiftool path
local LrDialogs = import 'LrDialogs'
local LrFileUtils = import 'LrFileUtils'
local LrTasks = import 'LrTasks'

-- function validateExifToolPath() -- commented out due to same functionality appearing to exist in externalapps.lua
--     -- Load the path from the backing file and set it to t.exifToolPath.
--     t.exifToolPath = app:getPref('exifToolApp')
    
--     -- Check if there's a valid path in t.exifToolPath.
--     if t.exifToolPath and LrFileUtils.exists(t.exifToolPath) then
--         return true
--     end
    
--     -- If not, check the default location within the plugin directory.
--     local defaultPath = LrPathUtils.child(_PLUGIN.path, "exiftool.exe")
--     if LrFileUtils.exists(defaultPath) then
--        t.exifToolPath = defaultPath
--        app:setPref('exifToolApp', defaultPath)  -- Save this path to the backing file.
--        return true
--     end
    
--     -- If ExifTool is still not found, prompt the user to specify its location.
...
--             t.exifToolPath = selectedPath
--             app:setPref('exifToolApp', selectedPath)  -- Save this new path to the backing file.
--             return true
--         end
--     end
    
--     LrDialogs.message("ExifTool not found", "Please ensure ExifTool is installed and specify its location.", "critical")
--     return false
-- end
...
-- Define the function to log path to a file
function logPathToFile(path, description)
    
    local logFilePath = "C:\\Users\\Seb\\Desktop\\Lightroom_prefspath.txt"
    
    local file = io.open(logFilePath, "a")
    if file then
        file:write(description, ": ", path, "\n")
        file:close()
        LrDialogs.showBezel("Path logged to: " .. logFilePath, 5)
    else
        LrDialogs.showError("Could not write to log file.")
    end
end
...
function Lightroom:computeActivePresetDir()
    ...
    local function computePresetDir()
        local manualOverridePath = prefs.manualOverridePath
        if manualOverridePath and #manualOverridePath > 0 then
            if LrFileUtils.exists(manualOverridePath) == "directory" then
                if not prefs.suppressNonFinalDialogs then
                    LrDialogs.message("Debug: Manual Override Path", manualOverridePath)
                end

                return manualOverridePath
            else
                LrDialogs.showError("The manual override path set does not exist or is not a directory.")
                return nil
            end
        end

        local prefsPath = LrPathUtils.child(LrPathUtils.getStandardFilePath('appData'), '/Preferences')
        logPathToFile(prefsPath, "Path to Master Prefs file")
        LrDialogs.message("Debug: Path to Master Prefs file", prefsPath)

        local largestFileSize = 0
        local prefsFile = nil

        for file in LrFileUtils.files(prefsPath) do
            local attributes = LrFileUtils.fileAttributes(file)
            if attributes then
                local fileSize = attributes.fileSize
                if fileSize and fileSize > largestFileSize then
                    largestFileSize = fileSize
                    prefsFile = file
                end
            else
                LrDialogs.showError("Failed to get file attributes for " .. tostring(file))
                return nil
            end
        end

        if not prefsFile then
            LrDialogs.showError("No preferences file found.")
            return nil
        end
    ...
    local status, result = pcall(computePresetDir)
    if not status then
        LrDialogs.showError("An error occurred: " .. tostring(result))
        return nil
    end
    ...
    if result then
        ...
        LrDialogs.showError("Unable to determine preset directory. Please set it manually.")
        local result = LrDialogs.runOpenPanel({
        ...
        if result and #result > 0 then
            manualOverridePath = result[1]
            prefs.manualOverridePath = manualOverridePath
            return manualOverridePath
        else
