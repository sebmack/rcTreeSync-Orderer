...
-- Assuming you've imported LrPrefs
...
function Service:cleanup( status, message )
    ...
    LrFunctionContext.callWithContext( "service cleanup finish", function( context )
        ...
        -- New Logic for MessageBox display starts here
        local showMessageBox = true
        local actionPrefKey
        local buttons = nil
        
        local userSuppressedAll = self.finalDialogBoxActionPrefKey == 'skip' -- Retrieve user preference for suppressing all dialog boxes
        local showOnlyFinal = prefs.suppressNonFinalDialogs -- Retrieve user preference for showing only final dialog box
    
        if userSuppressedAll or showOnlyFinal then
            showMessageBox = false
        end
    
        -- New Logic for MessageBox display ends here
        local answer = "skip"
        if showMessageBox then
            answer = app:show{ info=msg, buttons=buttons, actionPrefKey=actionPrefKey } -- no longer using error/warning specific dialog boxes for final presentation
