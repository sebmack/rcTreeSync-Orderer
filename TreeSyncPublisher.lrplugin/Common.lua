        ...
        elseif #colls2 == 1 then -- photo is in exactly one relevant collection - not ambiguous.
            ...
            if not coll then
                ...
                if isSample then
                    ...
                    local function compareWithSelectedPath(treeNodes, currentPath, leafPath)
                        for _, node in ipairs(treeNodes) do
                            if node.checked then
                                local nextPath = ""
                                if currentPath and #currentPath then
                                    nextPath = currentPath .. '\\' .. node.label
                                else
                                    nextPath = node.label
                                end
                                
                                if leafPath == nextPath then
                                    return true
                                end
                                if #node.children then
                                    if compareWithSelectedPath(node.children, nextPath, leafPath) then
                                        return true
                                    end
                                end
                            end
                    ...
                    local newColls = {}
                    for _, collectionItem in ipairs(colls) do
                        if compareWithSelectedPath(es.treeData, nil, collectionItem.title) then
                            table.insert(newColls, collectionItem)
                        end
                    end
...
function syncCollSet( srcSet, destSet, smartCollsToo )
    ...
    local function syncColl( fromColl, toSet, toLookup, toLookup2 )
        ...
        if not toColl then
            ...
            if isSmart then -- *from* coll is smart.
                ...
                if status then
                    -- LrDialogs.message(toColl:getName(), "nAdded: "..tostring(nAdded).."\nnRemoved: "..tostring(nRemoved))
    ...
    local function syncSet( fromSet, toSet )
        ...
        for i, v in ipairs( fromSet:getChildCollections() ) do -- breadth first
            -- LrDialogs.message("sync collection: ", v:getName())
    ...
    --     nCollSetTotal
    -- )
    local summary = str:fmtx( "                       Sets         Collections\nUnchanged:        0                ^1\nChanged:           0                ^2\nNew:                 ^3                ^4\nDeleted:             ^5                ^6\n",
        nCollSame,
        nCollModified,
        nCollSetAdded,
        nCollAdded,
    ...
    local function purgeFolderSet( fromFolder, toSet )
        ...
        for i, c in ipairs( toCollSets ) do
            ...
            if not fromCollSet then
                ...
                if s then
                    app:log( "Deleted collection set: ^1", setName )
                    nCollSetRemoved = nCollSetRemoved + 1
                else
                    app:logE( m )
                end
...
function Common.syncPhotos( call, photoMatch, autoOrder )
    ...
    for i, tspSrv in ipairs( tspSrvs ) do
        repeat
            ...
            app:log()
            app:log( "Considering tsp publish service: ^1", srvName )    -- srvName is just plugin-name, "tree sync"
            ...
            if es.destTreeType == 'coll' then
                ...
                -- LrDialogs.message("masterCollSet: ", masterCollSet:getName())
            ...
            if doPhotoMatch then -- photo matching is a go..
                ...
                if es.destTreeType == 'coll' then
                    ...
                    -- nErrors, colls, stats = syncCollSet( catalog, tspSrv, es.smartCollsToo ) -- ideally, ordering and matching could be done in the same loop - maybe one day.. ###3.
                elseif es.destTreeType == 'folder' then
...
function quickSyncCollSet( srcSet, destSet, smartCollsToo )
    local nErrors = app:getErrorCount()
    local colls = {} -- to publish
    local nCollAdded = 0
    local nCollRemoved = 0
...
function Common.quickSyncPhotos( call, photoMatch, autoOrder )
    app:callingAssert( call ~= nil, "call is nil" )
    app:callingAssert( photoMatch ~= nil, "pm is nil" )
    app:callingAssert( autoOrder ~= nil, "ao is nil" )
    local pubSrvs
    ...
    for i, tspSrv in ipairs( tspSrvs ) do
        repeat
            ...
            if photoMatch then
                --  my coding...
                -- LrDialogs.message("es.destTreeType: ", es.destTreeType)
                -- es.destTreeType = "coll"
                -- doPhotoMatch = true
                --  end
                if es.photosToo then -- user dictated photo-matching.
            ...
            if doPhotoMatch then -- photo matching is a go..
                ...
                if es.destTreeType == 'coll' then
                    assert( masterCollSet, "no master coll set" )
                    nErrors, colls, stats = quickSyncCollSet( masterCollSet, tspSrv, es.smartCollsToo ) -- ideally, ordering and matching could be done in the same loop - maybe one day.. ###3.
                elseif es.destTreeType == 'folder' then
                    local srcFolder = cat:getFolderByPath( es.sourcePath, true ) -- bypass cache.
                    if srcFolder then
                        nErrors, colls, stats = syncFolderHierarchy( srcFolder, tspSrv )
...
function fullSyncCollSet( srcSet, destSet, smartCollsToo, collPath, selection )
    local nErrors = app:getErrorCount()
    local colls = {} -- to publish
    local nCollAdded = 0
    local nCollRemoved = 0
    ...
    local function syncSet( fromSet, toSet, currentCollPath )
        local toChildColls = toSet:getChildCollections() -- array
        local toChildCollsLookup = {}
        for j, c in ipairs( toChildColls ) do
            toChildCollsLookup[c:getName()] = c
        ...
        for i, v in ipairs( fromSet:getChildCollections() ) do -- breadth first
            local fullCollPath = currentCollPath .. "/" .. v:getName()

            -- LrDialogs.message("sync collection path: ", fullCollPath.."\n"..tostring(selection[fullCollPath]))
            if selection["AllSelection"] or selection[fullCollPath] then
                syncColl( v, toSet, toChildCollsLookup, toChildCollSetsLookup )
            end
        end
        for i, v in ipairs( fromSet:getChildCollectionSets() ) do -- depth second
            ...
            if toChildSet then
                syncSet( v, toChildSet, currentCollPath .. "/" .. setName)
                nCollSetTotal = nCollSetTotal + 1
            end
        end
    end
    assert( destSet, "no dest set" )
    syncSet( srcSet, destSet, collPath ) -- add collections, sets, and add/remove photos in existing collections to match.
...
function Common.fullSyncPhotos( call, photoMatch, autoOrder, selection )
    app:callingAssert( call ~= nil, "call is nil" )
    app:callingAssert( photoMatch ~= nil, "pm is nil" )
    app:callingAssert( autoOrder ~= nil, "ao is nil" )
    local pubSrvs
    ...
    for i, tspSrv in ipairs( tspSrvs ) do
        repeat
            ...
            if doPhotoMatch then -- photo matching is a go..
                ...
                if es.destTreeType == 'coll' then
                    assert( masterCollSet, "no master coll set" )
                    nErrors, colls, stats = fullSyncCollSet( masterCollSet, tspSrv, es.smartCollsToo, srvName, selection ) -- ideally, ordering and matching could be done in the same loop - maybe one day.. ###3.
                elseif es.destTreeType == 'folder' then
                    local srcFolder = cat:getFolderByPath( es.sourcePath, true ) -- bypass cache.
                    if srcFolder then
                        nErrors, colls, stats = syncFolderHierarchy( srcFolder, tspSrv )
...
function syncPhotosBySelectedOption( srcSet, destSet, smartCollsToo, selection, rootPath )
    local nErrors = app:getErrorCount()
    local colls = {} -- to publish
    local nCollAdded = 0
    local nCollRemoved = 0
    ...
    local function syncSet( fromSet, toSet, currentPath )
        local toChildColls = toSet:getChildCollections() -- array
        local toChildCollsLookup = {}
        for j, c in ipairs( toChildColls ) do
            toChildCollsLookup[c:getName()] = c
        ...
        for i, v in ipairs( fromSet:getChildCollections() ) do -- breadth first
            local fullPath = v:getName()
            if currentPath ~= "" then
                fullPath = currentPath .. "/" .. fullPath
            end
            -- LrDialogs.message(toSet:getName().."service: / "..fullPath, tostring(selection[fullPath]))
            if selection[fullPath] then
                syncColl( v, toSet, toChildCollsLookup, toChildCollSetsLookup )
            end
        end
        for i, v in ipairs( fromSet:getChildCollectionSets() ) do -- depth second
            ...
            if selection[fullPath] then
                if not toChildSet then
                    local s, m = cat:update( 30, "Create collection set "..setName, function( context, phase )
                        ...
                        -- may be phase 1 (if pre-coll-deletion was required) or phase 2 (if not).
                        toChildSet = destSet:createPublishedCollectionSet( setName, toSet, false )  -- default is false
                    end )
                    if s then
                        if toChildSet then
                            app:log( "Created collection set." )
    ...
    assert( destSet, "no dest set" )
    purgeSet( srcSet, destSet ) -- remove extraneous collections & sets (but not photos).
    syncSet( srcSet, destSet, rootPath ) -- add collections, sets, and add/remove photos in existing collections to match.
...
function Common.syncPhotosBySelectedOption( call, photoMatch, autoOrder, selection )
    app:callingAssert( call ~= nil, "call is nil" )
    app:callingAssert( photoMatch ~= nil, "pm is nil" )
    app:callingAssert( autoOrder ~= nil, "ao is nil" )
    local pubSrvs
    ...
    for i, tspSrv in ipairs( tspSrvs ) do
        repeat
            ...
            if es.destTreeType == 'coll' then
                masterCollSet, mPath = Common.getMasterCollectionSet( es ) -- works whether psrv or cat set.
                if masterCollSet then
                    app:logW( "Got master collection set: ^1", mPath )
                    if mPath ~= "Catalog" then
                        mPath = "All Select/" .. string.gsub(mPath, "\\", "/")
                    else
                        mPath = "All Select"
                    end
                else
                    app:log("m path 2: ^1", mPath)
                    app:logW( "Master collection set not found, ID: ^1", es.collSetId )
                    mPath = "All Select"
                    break
                end
            ...
            if doPhotoMatch then -- photo matching is a go..
                ...
                if es.destTreeType == 'coll' then
                    assert( masterCollSet, "no master coll set" )
                    nErrors, colls, stats = syncPhotosBySelectedOption( masterCollSet, tspSrv, es.smartCollsToo, selection, mPath ) -- ideally, ordering and matching could be done in the same loop - maybe one day.. ###3.
                elseif es.destTreeType == 'folder' then
                    local srcFolder = cat:getFolderByPath( es.sourcePath, true ) -- bypass cache.
                    if srcFolder then
                        nErrors, colls, stats = syncFolderHierarchy( srcFolder, tspSrv )
...
function Common.syncPhotosByCollection( call, service, selection )
    app:callingAssert( call ~= nil, "call is nil" )
    
    local es = service:getPublishSettings()["< contents >"] or error( "where's the pub-srv settings?" )
    es.destTreeType = "coll"
    local masterCollSet, mPath = Common.getMasterCollectionSet( es ) -- works whether psrv or cat set.
    local srvName = service:getName()
    if masterCollSet then
        app:logV( "Got master collection set: ^1", mPath )
    else
        app:logW( "Master collection set not found, ID: ^1", es.collSetId )
    end
    -- LrDialogs.message("mPath: ", mPath)
    local nErrors, colls, stats = syncPhotosBySelectedOption( masterCollSet, service, true, selection, "" ) -- ideally, ordering and matching could be done in the same loop - maybe one day.. ###3.
    
    return nErrors, colls, stats
end


function Common.syncNormalCollectionToPublishCollection( call, selection )
    app:callingAssert( call ~= nil, "call is nil" )
    local pubSrvs
    local nUpd = 0
    local pubCollSet = {}
    ...
    for i, tspSrv in ipairs( tspSrvs ) do
        ...
        if es.destTreeType == "coll" then
            ...
            -- assert( masterCollSet, "no master coll set" )
            local nErrors, colls, stats = syncPhotosBySelectedOption( masterCollSet, tspSrv, es.smartCollsToo, selection, mPath ) -- ideally, ordering and matching could be done in the same loop - maybe one day.. ###3.
