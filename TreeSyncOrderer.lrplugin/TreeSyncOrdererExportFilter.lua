...
function TreeSyncOrdererExportFilter:updateFilterStatusMethod( name, value )
    ...
    app:call( Call:new{ name=str:fmtx( "^1 - Update Filter Status", self.filterName ), async=true, guard=App.guardSilent, main=function( context )
        ...
        repeat -- once
            ...
            --    break
            --else
                props[self.enablePropName] = true -- make sure a rogue disable preset doesn't kill the filter..
                app:assurePrefSupportFile( props.pluginManagerPreset ) -- ###0 - I guess: I don't think there is anything relevant, but does not hurt..
...
function TreeSyncOrdererExportFilter:shouldRenderPhotoMethod( photo )
    ...
    if str:is( settings.LR_publish_connectionName ) then -- being called from a publish service. (will be 'Unknown..' if left blank).
        return true -- reminder: whether a mock publish or true publish depends on skip-render function in main processing loop below.
    --else -- ordinary export - ignore fprop: same as TSO v2 and v1.
    end
    -- regular export:
...
function TreeSyncOrdererExportFilter:postProcessRenderedPhotosMethod()
    ...
    if str:is( exportSettings.LR_publish_connectionName ) then -- being called from a named publish service. It seems this property is auto-populated even if blank.
        app:logV( "Publish connection name: ^1", exportSettings.LR_publish_connectionName )
        if self.pubCollId == nil then
            self.pubCollId = fprops:getPropertyForPlugin( 'com.robcole.lightroom.TreeSyncPublisher', 'pseudoExportForSortOrdering', true ) -- force read.
        end
        if self.pubCollId then
            app:log( "Publishing pseudo-export for auto-ordering - collection ID: ^1", self.pubCollId ) -- *auto*-ordering, right? ###1
        else
            app:logV( "Normal publishing pass-through - no ordering.." )
            self:passRenditionsThrough()
            return
        end
    else -- ordinary export - always ena..
        app:logV( "Ordinary export, non-publishing, presumably for ordering purposes.." )
    end
    ...
    app:service{ name=str:fmtx( "^1 - Post-Process Rendered Photos", self.filterName ), main=function( call ) -- called from task.
        ...
        if str:is( exportSettings.LR_publish_connectionName ) then
            assert( self.pubCollId, "no pub-coll ID" )
            local ps = PublishServices:new()
            src = ps:getCollectionByLocalIdentifier( self.pubCollId ) or error( "no pub-coll for ID" ) -- auto-init not tooo lengthy..
            if src then
                ...
                app:log( "Considering publish collection, name: ^1, type: ^2", srcName, srcType )
            else
                app:logE( "No publish collection w/ID: ^1", self.pubCollId )
                return
            end
        else -- export
            ...
            srcName = src:getName()
            srcType = cat:getSourceType( src )
            app:log( "Considering active export source, name: ^1, type: ^2", srcName, srcType )
        end
        ...
        for r1, r2 in filterContext:renditions( renditionOptions ) do
            ...
            if s then
                ...
                -- Note: if it's successfully skipped, Lr won't be considering whether it's "done".
                nSkipped = nSkipped + 1
            else
                nOther = nOther + 1
        ...
        if self.pubCollId then
            if nSkipped > 0 then
                app:log()
                app:log( "^1 skipped in the interest of mock-publishing for auto-ordering sake.", str:nItems( nSkipped, "renditions" ) )
                app:log()
            elseif nOther then
                app:log()
                app:logW( "No renditions skipped in the interest of mock-publishing for auto-ordering sake." )
                app:log()
            else
                app:logV( "*** Hmm - no renditions in collection, so no auto-ordering of collection - seems odd.." )
            end
        else -- manual ordering export. reminder: normal publish passes through above an never reaches the skip loop, hopefully.
            if nSkipped > 0 then
                app:log()
                app:log( "^1 skipped in the interest of manual exporting for sort ordering sake.", str:nItems( nSkipped, "renditions" ) )
                app:log()
            elseif nOther then
                app:log()
                app:logW( "No renditions skipped in the interest of manual exporting for sort ordering sake." )
                app:log()
            else
                app:logV( "*** Hmm - no renditions in collection, so no ordering of collection - seems odd.." )
            end
        end

        local index = 0
        -- reminder: src-id will be nil if src is lr-folder.
        local orderSet = { { srcId=src.localIdentifier, updated=LrDate.currentTime() } } -- srcName } -- useful for human inspection..
        app:log()
        ...
        if src.localIdentifier then -- typically this would be a collection (definitely not a folder), but could also be a collection set or publish service.
            name[2] = collections:getFullCollPath( src ) -- src.localIdentifier
        ...
        fprops:setPropertyForPlugin( nil, name, orderSet ) -- this plugin.
...
function TreeSyncOrdererExportFilter.shouldRenderPhoto( exportSettings, photo )
    ...
    local shouldRender = filter:shouldRenderPhotoMethod( photo )
    -- if shouldRender then
       
    --     local es = exportSettings['< contents >']
    --     local photoName = photo:getFormattedMetadata("fileName")
    --     local sourcePath = photo:getRawMetadata("path")
    --     -- local getFormattedMetadata = photo:getFormattedMetadata("altTextAccessibility")
    --     local missing = LrFileUtils.exists(sourcePath)

    --     local missingOriginals = fprops:getPropertyForPlugin( 'com.robcole.TreeSyncPublisher', 'missingOriginals', "" ) -- force read.
    --     local destinationFolder = fprops:getPropertyForPlugin( 'com.robcole.TreeSyncPublisher', 'destinationFolder', "" ) -- force read.
    --     local collectionId = fprops:getPropertyForPlugin( 'com.robcole.TreeSyncPublisher', 'collectionId', 0 ) -- force read.
    --     local collName = fprops:getPropertyForPlugin( 'com.robcole.TreeSyncPublisher', 'collName', "" ) -- force read.
    --     local ps = PublishServices:new()
    --     local collection = PublishServices:getCollectionByLocalIdentifier( collectionId )

    --     if not missing then
    --         -- LrDialogs.message("coll type: ", type(collection))
    --         -- LrDialogs.message("coll type: ", type(collection))
    --         -- LrDialogs.message("coll name: ", tostring(collection:getName()))
    --         if collection == nil or missingOriginals == "All" then
    --             app:log(str:fmtx("^1 in ^2 doesn't exists in the ^3.", photoName, collName, destinationFolder))
    --             return false
    --         end

    --         local destinationFilePath = destinationFolder .. "\\" .. photoName
    --         local isExist = LrFileUtils.exists(destinationFilePath)
    --         -- LrDialogs.message("isExist: ", tostring(isExist))

    --         -- local originCaptureTime = photo:getRawMetadata('com.adobe.captureTime')
    --         -- local originCameraSN = photo:getRawMetadata('com.adobe.cameraSN')
    --         -- LrDialogs.message(photoName, tostring(originCameraSN))


    --         if isExist then
    --             local text = str:fmtx("^1 in ^2 already exists in the ^3.", photoName, collection:getName(), destinationFolder)
    ...
    --                 end)
    --                 app:log(str:fmtx("^1 is removed from ^2", photoName, collection:getName()))
    --             else
    --                 app:log(str:fmtx("^1 can't be removed from ^2 because it is smart collection.", photoName, collection:getName()))
    --             end
    --             app:log(text)
    --             -- catalog:withWriteAccessDo("Set Metadata", function()
    --             --     photo:setRawMetadata('altTextAccessibility', "rcTreeSyncMissingOriginalDestinationImageExists")  -- Example: setting the rating
    --             -- end)

    --             if missingOriginals == "CopyExists" or missingOriginals == "All" then
    --                 return false
    --             end
    --         else
    --             -- local collectionName = cat:getSourceName( collection )
    --             local collectionName = collection:getName()
    --             local text = str:fmtx("^1 in ^2 doesn't exist in the ^3.", photoName, collectionName, destinationFolder)
    --             if missingOriginals == "Prompt" then
    --                 LrDialogs.message( "Not found " .. photoName, text )
    --             end
    --             app:log(text)
    --             -- catalog:withWriteAccessDo("Set Metadata", function()
    --             --     photo:setRawMetadata('altTextAccessibility', "rcTreeSyncMissingOriginal")  -- Example: setting the rating
    --             -- end)
    --             if missingOriginals == "All" then
    --                 return false
    --             end
    --         end
    --     else
    --         -- catalog:withWriteAccessDo("Set Metadata", function()
    --         --     photo:setRawMetadata('altTextAccessibility', '')  -- Example: setting the rating
    --         -- end)
