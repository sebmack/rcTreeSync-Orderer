...
_G.TreeSyncOrdererObjectFactory = require( 'TreeSyncOrdererObjectFactory' )    -- class of object used to create objects of classes not mandated by the framework.
_G.objectFactory = TreeSyncOrdererObjectFactory:new()               -- object used to create objects of classes not mandated by the framework.
...
_G.TreeSyncOrdererManager = require( "TreeSyncOrdererManager" )
_G.TreeSyncOrdererExport = require( "TreeSyncOrdererExport" )
_G.TreeSyncOrdererExportFilter = require( "TreeSyncOrdererExportFilter" )
_G.TreeSyncOrderer = require( "TreeSyncOrderer" )



--   I N I T I A T E   A S Y N C H R O N O U S   I N I T   A N D   B A C K G R O U N D   T A S K
TreeSyncOrdererManager.initPrefs() -- remember to include all dependent plugin prefs (plugin generator is not helping in this regard, yet)
tso = TreeSyncOrderer:new()
background = ExtendedBackground:new()
autoMode = false
app:initDone() -- synchronous init done.
background:start() -- by default just does async init then quits - consider using pref to enable/disable background if optional, else force continuation into background processing...

-- INITIALIZE PREFERENCE FOR MANUAL OVERRIDE PATH

local LrPrefs = import 'LrPrefs'
local prefs = LrPrefs.prefsForPlugin()

if not prefs.manualOverridePath then
    prefs.manualOverridePath = ''
end
